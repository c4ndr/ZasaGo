...
import {
  ...
  Bell, HelpCircle, ShieldCheck, ClipboardList,
  CheckCircle, Moon, Headphones, FileText, AlertCircle,
} from 'lucide-react'
...
const ROLE_CFG = {
  ...
  mitra:     { label: 'Mitra',         bg: 'bg-blue-100',    text: 'text-blue-700' },
  pelanggan: { label: 'Pelanggan',     bg: 'bg-indigo-100',  text: 'text-indigo-700' },
  penjual:   { label: 'Penjual',       bg: 'bg-pink-100',    text: 'text-pink-700' },
}
...
export default function Akun() {
  ...
  // Sync profil dari API saat mount
  useEffect(() => {
    api.get('/profil')
      .then(({ data }) => {
        setUser(data)
  ...
  const roleCfg = ROLE_CFG[user?.role] || ROLE_CFG.pelanggan
  ...
  const riwayatPath =
    user?.role === 'admin'   ? '/admin/pesanan' :
    user?.role === 'mitra'   ? '/mitra/pesanan' :
    user?.role === 'penjual' ? '/penjual/pesanan' :
    '/pelanggan/riwayat'
    ...
    <div className="pb-4">
      ...
      >
        {[
          ...
          <div key={i} style={{ position:'absolute', borderRadius:'50%', pointerEvents:'none', ...s, background: s.bg }} />
        ))}

        <div className="relative z-10 flex flex-col items-center text-center">
          <div
            style={{ background: 'rgba(255,255,255,0.2)', boxShadow: '0 0 0 4px rgba(255,255,255,0.3)' }}
          ...
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold mt-2 ${roleCfg.bg} ${roleCfg.text}`}>
            {roleCfg.label}
          </span>
      ...
      {!syncing && !profileComplete && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-600">Isi nomor HP, alamat, dan jenis kelamin untuk pengalaman lebih baik</p>
          </div>
          <button onClick={() => setModal('edit')} className="text-xs font-bold text-amber-700 bg-amber-100 hover:bg-amber-200 px-2.5 py-1 rounded-lg transition-colors">
            Lengkapi
          </button>
        </div>
      )}
      ...
      <section className="card mb-4">
        ...
        <div className="divide-y divide-gray-50">
          ...
          <div className="flex items-center gap-3 w-full py-3">
            <div className="w-9 h-9 bg-primary-50 rounded-xl flex items-center justify-center shrink-0"><Bell className="w-4 h-4 text-primary-600" /></div>
            <div className="flex-1 text-left"><p className="text-sm font-semibold text-gray-800">Notifikasi</p><p className="text-xs text-gray-400">Push notifikasi aktif</p></div>
            <Toggle on={notifOn} onToggle={() => setNotifOn((v) => !v)} />
          </div>

          <div className="flex items-center gap-3 w-full py-3">
            <div className="w-9 h-9 bg-primary-50 rounded-xl flex items-center justify-center shrink-0"><Moon className="w-4 h-4 text-primary-600" /></div>
            <div className="flex-1 text-left"><p className="text-sm font-semibold text-gray-800">Mode Gelap</p><p className="text-xs text-gray-400">Tampilan gelap / terang</p></div>
      ...
      <section className="card mb-4">
        ...
        <div className="divide-y divide-gray-50">
          {[
            { icon: ClipboardList, label: 'Riwayat Pesanan',   sub: 'Lihat semua transaksi',    onClick: () => navigate(riwayatPath) },
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditProfile} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input className="input-field" placeholder="Nama lengkap Anda" value={editForm.name}
              ...
              <div>
                ...
                <input type="tel" className="input-field" placeholder="08xxxxxxxxxx" value={editForm.phone}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl">
            ...
            <form onSubmit={handleChangePassword} className="px-6 py-5 space-y-4">
              {[
                { key: 'current_password',      label: 'Password Lama',            placeholder: 'Masukkan password lama' },
                { key: 'password',              label: 'Password Baru',            placeholder: 'Min. 8 karakter' },
                { key: 'password_confirmation', label: 'Konfirmasi Password Baru', placeholder: 'Ulangi password baru' },
              ].map(({ key, label, placeholder }) => (
                <div key={key}>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">{label}</label>
                  <div className="relative">
                    <input type={showPass[key] ? 'text' : 'password'} className="input-field pr-11"
                      placeholder={placeholder} value={passForm[key]}
                      onChange={(e) => setPassForm({ ...passForm, [key]: e.target.value })} required />
