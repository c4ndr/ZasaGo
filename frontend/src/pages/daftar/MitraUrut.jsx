...
const JENIS_URUT = ['Urut Badan', 'Urut Kaki', 'Urut Kepala', 'Full Body', 'Refleksi']
...
export default function DaftarMitraUrut() {
  const navigate = useNavigate()
  const { setAuth } = useAuthStore()
  const [form, setForm] = useState({
    name: '', email: '', phone: '', address: '',
    password: '', password_confirmation: '',
    pengalaman: '', jam_mulai: '08:00', jam_selesai: '20:00',
    home_visit: true, ktp_image: '', sim_image: '',
  })
  const [jenisUrut, setJenisUrut] = useState([])
  const [showPass, setShowPass] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const toggleJenis = (j) => setJenisUrut((prev) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const bio = JSON.stringify({
        ...
        jenis_urut:    jenisUrut,
        jam_operasional: `${form.jam_mulai} – ${form.jam_selesai}`,
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="w-full max-w-sm mx-auto">
        ...
        <div className="text-center mb-5">
          ...
          <h1 className="text-xl font-black text-gray-900">Daftar Mitra Urut/Pijat</h1>
          <p className="text-gray-400 text-sm mt-1">Layani jasa pijat & urut ke rumah pelanggan</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Jenis Urut yang Dikuasai</label>
              <div className="flex flex-wrap gap-2">
                {JENIS_URUT.map((j) => (
                  <button key={j} type="button" onClick={() => toggleJenis(j)}
                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-colors ${
                      jenisUrut.includes(j) ? 'bg-primary-600 text-white border-primary-600' : 'bg-gray-50 text-gray-600 border-gray-200 hover:border-primary-300'
                    }`}>
                    {j}
                  </button>
                ))}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Bisa Home Visit?</label>
              <div className="flex gap-3">
                {[true, false].map((v) => (
                  <button key={String(v)} type="button" onClick={() => setForm({ ...form, home_visit: v })}
                    className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${
                      form.home_visit === v ? 'bg-primary-600 text-white border-primary-600' : 'bg-gray-50 text-gray-600 border-gray-200'
                    }`}>
                    {v ? 'Ya' : 'Tidak'}
                  </button>
                ))}
            ...
            <ImageUpload label="Foto Sertifikat / Bukti Keahlian (opsional)" value={form.sim_image} onChange={(v) => setForm({ ...form, sim_image: v })} />
