...
export default function HomeMyOrdersPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--k-bg)', paddingBottom: 40 }}>
      ...
      {/* Header */}
      <div style={{ background: SVC.zasahome.bg, padding: '52px 20px 24px', position: 'relative', overflow: 'hidden', boxShadow: svcShadow(SVC.zasahome.rgb, true) }}>
        <Gloss />
        <div style={{ position: 'relative' }}>
          <button onClick={() => navigate('/home')} style={{ background: 'var(--k-surface)', border: 'none', color: SVC.zasahome.fg, fontSize: 13, cursor: 'pointer', marginBottom: 16, padding: '6px 14px', borderRadius: 20, boxShadow: `0 3px 8px rgba(${SVC.zasahome.rgb},0.22), inset 0 1.5px 0 rgba(255,255,255,0.9)` }}>
            ← ZasaHome
          </button>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: SVC.zasahome.fg, marginBottom: 4 }}>Pesanan Saya</h1>
          <p style={{ color: SVC.zasahome.fg, opacity: 0.75, fontSize: 13 }}>{orders.length} total pesanan</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', background: 'var(--k-card)', borderBottom: '1px solid var(--k-border)' }}>
        {[
          ...
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            ...
            borderBottom: tab === t.key ? `2.5px solid ${SVC.zasahome.fg}` : '2.5px solid transparent',
            fontWeight: tab === t.key ? 700 : 400, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          }}>
            {t.label}
            {t.count > 0 && (
              <span style={{ fontSize: 11, fontWeight: 800, padding: '1px 7px', borderRadius: 20, background: tab === t.key ? SVC.zasahome.bg : 'var(--k-input)', color: tab === t.key ? SVC.zasahome.fg : 'var(--k-muted)' }}>{t.count}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '16px 16px 0' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--k-muted)', animation: 'fadeUp 0.4s ease' }}>
            ...
            {tab === 'active' && (
              <button onClick={() => navigate('/home')} style={{ marginTop: 8, padding: '10px 24px', borderRadius: 12, border: 'none', cursor: 'pointer', background: SVC.zasahome.bg, color: SVC.zasahome.fg, fontWeight: 700, boxShadow: svcShadow(SVC.zasahome.rgb) }}>
                Cari Layanan
              </button>
            )}
          </div>
    ...
    <button onClick={onClick} style={{
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        {order.scheduled_pickup_at && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 8, background: `rgba(${SVC.zasahome.rgb},0.08)`, marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTop: '1px solid var(--k-border)' }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 800, color: SVC.zasahome.fg }}>Rp {order.total_price.toLocaleString('id')}</p>
        </div>
      </div>
    </button>
  )
