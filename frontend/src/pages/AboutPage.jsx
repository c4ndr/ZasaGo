...
const MODULES = [
  { emoji: '🛵', name: 'ZasaGo',    desc: 'Ojek & pengiriman ekspres', color: 'var(--k-warn)',   rgb: '184,134,11', path: '/layanan' },
  { emoji: '🍔', name: 'ZasaFood',  desc: 'Pesan makanan dari merchant terdekat', color: 'var(--k-danger)', rgb: '192,67,92', path: '/layanan' },
  { emoji: '🛒', name: 'ZasaMart',  desc: 'Belanja produk dari pedagang lokal', color: 'var(--k-accent)', rgb: '46,125,91', path: '/layanan' },
  { emoji: '🏠', name: 'ZasaHome',  desc: 'Jasa layanan rumah & kebersihan', color: 'var(--k-primary)', rgb: '40,55,75', path: '/layanan' },
]
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {MODULES.map(m => (
              <button key={m.name} onClick={() => navigate(m.path)}
                ...
                <div style={{ width: 44, height: 44, borderRadius: 13, background: `rgba(${m.rgb},0.12)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{m.emoji}</div>
                <div style={{ flex: 1 }}>
        ...
        <div style={{ background: 'var(--k-card)', borderRadius: 16, padding: '18px', border: '1px solid var(--k-border)' }}>
          ...
          {INFO_ROWS.map((r, i) => (
            <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: i < INFO_ROWS.length - 1 ? 10 : 0, marginBottom: i < INFO_ROWS.length - 1 ? 10 : 0, borderBottom: i < INFO_ROWS.length - 1 ? '1px solid var(--k-border)' : 'none' }}>
              ...
              {r.href
                ? <a href={r.href} style={{ fontSize: 13, fontWeight: 700, color: 'var(--k-accent)', textDecoration: 'none' }}>{r.value}</a>
        ...
        <div style={{ background: 'linear-gradient(135deg,rgba(var(--k-accent-rgb),0.08),rgba(var(--k-accent-rgb),0.04))', borderRadius: 16, padding: '18px', border: '1px solid rgba(var(--k-accent-rgb),0.2)', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <button onClick={() => navigate('/daftar-mitra')}
              style={{ padding: '9px 16px', borderRadius: 10, border: 'none', background: 'var(--k-accent)', color: '#0C0C16', fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>
              Pelajari Lebih →
            </button>
          </div>
        </div>
