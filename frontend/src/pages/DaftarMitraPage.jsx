      ...
      <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#0C0C16,#1a1a2e)', borderRadius: 20, padding: '24px 20px', position: 'relative', overflow: 'hidden' }}>
          ...
          <button onClick={() => navigate('/register')}
            style={{ padding: '13px 24px', borderRadius: 12, border: 'none', background: 'var(--k-accent)', color: '#0C0C16', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>
            Daftar Jadi Mitra →
          </button>
        </div>
            ...
            {MITRA_TYPES.map(m => (
              <div key={m.name} style={{ background: 'var(--k-card)', borderRadius: 16, border: '1px solid var(--k-border)', overflow: 'hidden' }}>
                <div style={{ padding: '16px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: `rgba(${m.rgb},0.1)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>{m.emoji}</div>
                  <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              <div key={s} style={{ display: 'flex', gap: 8 }}>
                <span style={{ color: 'var(--k-primary)', fontSize: 13, flexShrink: 0 }}>›</span>
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--k-muted)', margin: 0 }}>
          Pertanyaan? Hubungi <a href="mailto:support@zasaqu.uk" style={{ color: 'var(--k-accent)', fontWeight: 700, textDecoration: 'none' }}>support@zasaqu.uk</a>
        </p>
      </div>
    </div>
  )
