...
export default function MitraAntarPenghasilan() {
  const [orders, setOrders]   = useState([])
  const [period, setPeriod]   = useState('bulan')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.get('/antar/pesanan').then((r) => setOrders(r.data.data || r.data))
  ...
  const total = filtered.reduce((s, o) => s + parseFloat(o.driver_earnings ?? o.penghasilan_mitra ?? 0), 0)
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
        {PERIODS.map((p) => (
          <button key={p.key} onClick={() => setPeriod(p.key)}
            className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${period === p.key ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'}`}>
            {p.label}
          </button>
        ))}
      </div>

      <div className="bg-gradient-to-r from-indigo-500 to-indigo-700 rounded-2xl p-5 text-white text-center">
        <p className="text-indigo-200 text-xs mb-1">Total Penghasilan</p>
        <p className="text-3xl font-black">{formatRupiah(total)}</p>
        <p className="text-indigo-200 text-xs mt-1">{filtered.length} pengiriman selesai</p>
      </div>
        ...
        {filtered.map((o) => (
          <div key={o.id} className="card flex justify-between items-center">
            <div>
              ...
              <p className="text-[11px] text-gray-400 truncate max-w-[160px]">{o.pickup_address} → {o.destination_address}</p>
              {o.distance_km && <p className="text-[10px] text-gray-300">{o.distance_km} km</p>}
