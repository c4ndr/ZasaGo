...
export default function MitraUrutPenghasilan() {
  ...
  const total    = filtered.reduce((s, o) => s + parseFloat(o.penghasilan_mitra ?? o.price ?? 0), 0)
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
        {PERIODS.map((p) => (
          <button key={p.key} onClick={() => setPeriod(p.key)}
            className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${period === p.key ? 'bg-white text-pink-600 shadow-sm' : 'text-gray-500'}`}>
            {p.label}
          </button>
        ))}
      </div>

      <div className="bg-gradient-to-r from-pink-500 to-pink-700 rounded-2xl p-5 text-white text-center">
        <p className="text-pink-200 text-xs mb-1">Total Penghasilan</p>
        <p className="text-3xl font-black">{formatRupiah(total)}</p>
        <p className="text-pink-200 text-xs mt-1">{filtered.length} pesanan selesai</p>
      </div>
        ...
        {filtered.map((o) => (
          <div key={o.id} className="card flex justify-between items-center">
            <div>
              ...
              <p className="text-[11px] text-gray-400">{o.pelanggan?.name} · {o.jenis_urut}</p>
