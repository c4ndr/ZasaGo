...
export default function MitraKebersihanAkun() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-teal-500 to-teal-700 rounded-2xl p-5 text-white flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center text-2xl font-bold">
          {(profil?.name || user?.name || '?')[0].toUpperCase()}
        </div>
        <div>
          <p className="font-bold text-lg">{profil?.name || user?.name}</p>
          <p className="text-teal-200 text-xs">Mitra Kebersihan · {profil?.mitra_profile?.is_available ? '🟢 Online' : '🔴 Offline'}</p>
        </div>
      </div>
      ...
      <div className="card space-y-3">
        <div className="flex justify-between items-center">
          ...
          {!editing
            ? <button onClick={() => setEditing(true)} className="flex items-center gap-1 text-xs text-teal-600"><Edit2 className="w-3.5 h-3.5" /> Edit</button>
            : <div className="flex gap-2">
                ...
                <button onClick={handleSave} disabled={saving} className="text-xs text-teal-600 flex items-center gap-1"><Save className="w-3.5 h-3.5" /> Simpan</button>
              </div>
          }
        </div>
        ...
        {editing ? (
          <div className="space-y-3">
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">Jenis Layanan</label>
              <textarea className="input-field resize-none" rows={3} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} placeholder="Contoh: Bersih rumah, kantor, kost" /></div>
          </div>
        ) : (
          <div className="space-y-2">
            {[
              ...
              { label: 'Layanan',  value: profil?.mitra_profile?.bio || '-' },
