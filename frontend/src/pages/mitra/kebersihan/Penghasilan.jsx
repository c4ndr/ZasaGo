...
export default function MitraKebersihanPenghasilan() {
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
        {PERIODS.map((p) => (
          <button key={p.key} onClick={() => setPeriod(p.key)}
            className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${period === p.key ? 'bg-white text-teal-600 shadow-sm' : 'text-gray-500'}`}>
            {p.label}
          </button>
        ))}
      </div>

      <div className="bg-gradient-to-r from-teal-500 to-teal-700 rounded-2xl p-5 text-white text-center">
        <p className="text-teal-200 text-xs mb-1">Total Penghasilan</p>
        <p className="text-3xl font-black">{formatRupiah(total)}</p>
        <p className="text-teal-200 text-xs mt-1">{filtered.length} pesanan selesai</p>
      </div>
        ...
        {filtered.map((o) => (
          <div key={o.id} className="card flex justify-between items-center">
            <div>
              ...
              <p className="text-[11px] text-gray-400">{o.pelanggan?.name} · {o.jenis_layanan}</p>
              {o.durasi_jam && <p className="text-[10px] text-gray-300">{o.durasi_jam} jam</p>}
