...
export default function MitraLaundryPenghasilan() {
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
        {PERIODS.map((p) => (
          <button key={p.key} onClick={() => setPeriod(p.key)}
            className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${period === p.key ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500'}`}>
            {p.label}
          </button>
        ))}
      </div>

      <div className="bg-gradient-to-r from-blue-500 to-blue-700 rounded-2xl p-5 text-white text-center">
        <p className="text-blue-200 text-xs mb-1">Total Penghasilan</p>
        <p className="text-3xl font-black">{formatRupiah(total)}</p>
        <p className="text-blue-200 text-xs mt-1">{filtered.length} pesanan selesai</p>
      </div>
        ...
        {filtered.map((o) => (
          <div key={o.id} className="card flex justify-between items-center">
            <div>
              ...
              {o.berat_kg && <p className="text-[10px] text-gray-300">{o.berat_kg} kg</p>}
