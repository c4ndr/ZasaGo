...
export default function MitraLaundryAkun() {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-700 rounded-2xl p-5 text-white flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-blue-200 text-xs">Mitra Laundry · {profil?.mitra_profile?.is_available ? '🟢 Online' : '🔴 Offline'}</p>
        </div>
      </div>
      ...
      <div className="card space-y-3">
        <div className="flex justify-between items-center">
          ...
          {!editing
            ? <button onClick={() => setEditing(true)} className="flex items-center gap-1 text-xs text-blue-600"><Edit2 className="w-3.5 h-3.5" /> Edit</button>
            : <div className="flex gap-2">
                ...
                <button onClick={handleSave} disabled={saving} className="text-xs text-blue-600 flex items-center gap-1"><Save className="w-3.5 h-3.5" /> Simpan</button>
              </div>
          }
        </div>
        ...
        {editing ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea className="input-field resize-none" rows={3} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} placeholder="Contoh: Laundry kiloan & satuan, antar jemput" />
            </div>
          </div>
        ) : (
          <div className="space-y-2 text-sm text-gray-600">
            {[
              ...
              { label: 'Deskripsi', value: profil?.mitra_profile?.bio || '-' },
