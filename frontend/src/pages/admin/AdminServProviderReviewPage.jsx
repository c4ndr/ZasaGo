...
function ReviewCard({ provider: p, onApprove, onReject }) {
  ...
  async function handleReject() {
    ...
    setBusy(true)
    await onReject(p.id, p.name, reason)
    setBusy(false)
    setShowReject(false)
  }
  ...
  return (
    <div style={{
      background: 'var(--k-card)', border: `1.5px solid rgba(${SVC.zasaserv.rgb},0.3)`,
      borderRadius: 18, overflow: 'hidden',
      boxShadow: `0 2px 12px rgba(${SVC.zasaserv.rgb},0.08)`,
    }}>
      {/* Header strip */}
      <div style={{ background: `rgba(${SVC.zasaserv.rgb},0.08)`, padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid rgba(${SVC.zasaserv.rgb},0.15)` }}>
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 20px', marginBottom: 14, fontSize: 13 }}>
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
            ...
            <span style={{ color: 'var(--k-text)', lineHeight: 1.4 }}>{p.address}</span>
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ color: 'var(--k-muted)', flexShrink: 0 }}>👤</span>
            <span style={{ color: 'var(--k-text)' }}>{p.user?.name}</span>
          </div>
          {p.phone && (
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <span style={{ color: 'var(--k-muted)', flexShrink: 0 }}>📞</span>
              <span style={{ color: 'var(--k-text)' }}>{p.phone}</span>
            </div>
          )}
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ color: 'var(--k-muted)', flexShrink: 0 }}>✉️</span>
            <span style={{ color: 'var(--k-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.user?.email}</span>
          </div>
          {p.open_time && (
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <span style={{ color: 'var(--k-muted)', flexShrink: 0 }}>🕐</span>
              <span style={{ color: 'var(--k-text)' }}>{p.open_time?.slice(0,5)} – {p.close_time?.slice(0,5)}</span>
            </div>
          )}
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ color: 'var(--k-muted)', flexShrink: 0 }}>📅</span>
            <span style={{ color: 'var(--k-text)', fontSize: 11 }}>Daftar: {fmtDate(p.created_at)}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
          ...
          {p.experience_years > 0 && (
            <span style={{ fontSize: 12, padding: '4px 12px', borderRadius: 20, background: 'var(--k-input)', color: 'var(--k-muted)', fontWeight: 600 }}>
          ...
          <div style={{ borderTop: '1px solid var(--k-border)', paddingTop: 14, marginBottom: 14 }}>
            {detailLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {info.description && (
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--k-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Deskripsi</p>
                    <p style={{ fontSize: 13, color: 'var(--k-text)', lineHeight: 1.6, fontStyle: 'italic' }}>"{info.description}"</p>
                  </div>
                )}

                {/* Spesialisasi */}
                {info.specializations?.length > 0 && (
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {info.specializations.map(s => (
                        <span key={s} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 20, background: 'rgba(var(--k-accent-rgb),0.1)', color: 'var(--k-accent)', fontWeight: 600 }}>{s}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {info.certificates?.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--k-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Sertifikat & Lisensi</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {info.certificates.map((c, i) => (
                        <p key={i} style={{ fontSize: 13, color: 'var(--k-text)' }}>🎖️ {c}</p>
                      ))}
                    </div>
                  </div>
                )}

                {/* Layanan yang sudah didaftarkan */}
                {info.all_services?.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--k-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
                      Layanan Terdaftar ({info.all_services.length})
                    </p>
                    <div style={{ background: 'var(--k-input)', borderRadius: 10, overflow: 'hidden' }}>
                      {info.all_services.map((s, i) => (
                        <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 12px', borderTop: i === 0 ? 'none' : '1px solid var(--k-border)' }}>
                          <div>
                            <p style={{ fontSize: 13, color: 'var(--k-text)', fontWeight: 600 }}>{s.name}</p>
                            <p style={{ fontSize: 11, color: 'var(--k-muted)' }}>per {s.unit}</p>
                          </div>
                          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--k-accent)' }}>Rp {Number(s.price).toLocaleString('id')}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {!info.description && !info.specializations?.length && !info.certificates?.length && !info.all_services?.length && (
        ...
        {showReject && (
          <div style={{ marginBottom: 14, padding: '14px', borderRadius: 12, background: 'rgba(var(--k-danger-rgb),0.05)', border: '1px solid rgba(var(--k-danger-rgb),0.2)' }}>
            ...
            <textarea value={reason} onChange={e => setReason(e.target.value)} rows={2} placeholder="Contoh: Data tidak lengkap, mohon lengkapi sertifikat dan foto..."
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button onClick={() => { setShowReject(false); setReason('') }} style={{ flex: 1, padding: '9px', borderRadius: 8, border: '1px solid var(--k-border)', background: 'var(--k-input)', color: 'var(--k-muted)', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                Batal
              </button>
        ...
        {!showReject && (
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={handleApprove} disabled={busy}
              style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: busy ? 'var(--k-border)' : SVC.zasaserv.fg, color: busy ? 'var(--k-muted)' : '#fff', fontWeight: 700, fontSize: 13, cursor: busy ? 'default' : 'pointer' }}>
...
// ── Halaman utama ─────────────────────────────────────────────────────────────
export default function AdminServProviderReviewPage() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    api.get(`/admin/serv/providers?status=pending&page=${page}`)
      .then(r => {
  ...
  async function handleApprove(id, name) {
    try {
      await api.post(`/admin/serv/providers/${id}/approve`)
      removeFromList(id)
      showToast('success', `✓ ${name} berhasil disetujui.`)
    } catch (err) { showToast('error', err.response?.data?.message || 'Gagal menyetujui.') }
  }

  async function handleReject(id, name, reason) {
    try {
      await api.post(`/admin/serv/providers/${id}/suspend`, { reason })
      removeFromList(id)
  ...
  return (
    <AdminLayout title="Review Provider ZasaServis">
      {toast && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, padding: '12px 20px', borderRadius: 12, fontSize: 14, fontWeight: 600, background: toastColor[toast.type] ?? SVC.zasaserv.fg, color: '#fff', maxWidth: 320, boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}>
          {toast.msg}
        </div>
      )}
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ margin: 0, fontSize: 13, color: 'var(--k-muted)' }}>
          Tinjau registrasi provider baru — periksa kelengkapan data sebelum menyetujui
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '80px 20px', background: 'var(--k-card)', borderRadius: 20, border: '1px solid var(--k-border)' }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--k-muted)' }}>Tidak ada provider yang menunggu review saat ini.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 800 }}>
          {providers.map(p => (
            <ReviewCard key={p.id} provider={p} onApprove={handleApprove} onReject={handleReject} />
          ))}
        </div>
      )}
