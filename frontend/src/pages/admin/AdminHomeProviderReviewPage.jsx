...
const CATEGORIES = {
  ...
  pijat:    { label: 'Pijat',     emoji: '💆' },
  cleaning: { label: 'Cleaning',  emoji: '🧹' },
  tukang:   { label: 'Tukang',    emoji: '🔨' },
...
function ReviewCard({ provider: p, onApprove, onReject }) {
  ...
  const services = detail?.all_services ?? []
          ...
          <div style={{ borderTop: '1px solid var(--k-border)', paddingTop: 14, marginBottom: 14 }}>
            {detailLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                      {[
                        ...
                        { label: 'Pending',      val: stats.pending_orders,   emoji: '⏳' },
                ...
                {services.length > 0 && (
                  <div>
                    <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--k-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
                      Layanan Terdaftar ({services.length})
                    </p>
                    <div style={{ background: 'var(--k-input)', borderRadius: 10, overflow: 'hidden' }}>
                      {services.map((sv, i) => (
                        <div key={sv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 12px', borderTop: i === 0 ? 'none' : '1px solid var(--k-border)' }}>
                          <div>
                ...
                {!info.description && !stats && !info.specializations?.length && !info.certificates?.length && services.length === 0 && (
...
// ── Halaman utama ─────────────────────────────────────────────────────────────
export default function AdminHomeProviderReviewPage() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    api.get(`/admin/home/providers?status=pending&page=${page}`)
      .then(r => {
  ...
  async function handleApprove(id, name) {
    try {
      await api.post(`/admin/home/providers/${id}/approve`)
      removeFromList(id)
  ...
  async function handleReject(id, name, reason) {
    try {
      await api.post(`/admin/home/providers/${id}/suspend`, { reason })
      removeFromList(id)
